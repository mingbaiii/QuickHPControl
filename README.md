...
一款轻量级的 HP 笔记本性能模式切换工具。**直连 BIOS 读写热控模式** —— 不注入任何进程、不加载 HP 的任何 DLL，只需管理员权限即可自由切换性能模式。**可完全替代 HP 官方 App 内的性能调节功能**，无需打开臃肿的 HP 软件，直接从系统托盘一键快速切换。
...
- **Windows 节能模式联动**：切换到省电模式时自动打开 Windows 11 的「节能模式」开关，
  切走时自动关闭。**只在切换的那一瞬间改一次**，之后用户手动改回来不会被覆盖。
